package graphimplementation;

import java.awt.EventQueue;
import java.util.List;

import dijkstra.Dijkstra;
import dijkstra.Vertex;
import view.MainWindow;

public class LeProgram
{

    public static void main(String[] args)
    {
//	 for (int i = 0; i < 26; i++)
//	 {
//	 AdjacentListGraph alg = new AdjacentListGraph();
//	 Vertex source = alg.nodes.get(14);
//	
//	 Dijkstra.computePaths(source);
//	 System.out.println("Source: " + source);
//	 for (Vertex node : alg.nodes)
//	 {
//	 System.out.println("Distance to " + node + ": " + node.minDistance);
//	 List<Vertex> path = Dijkstra.getShortestPathTo(node);
//	 System.out.println("Shortest path: " + path);
//	 }
//	 }

//	 AdjacentListGraph alg = new AdjacentListGraph();
//	 Vertex source = alg.nodes.get(23);
//	 Dijkstra.computePaths(source);
//	 System.out.println("Source: " + source);
//	 System.out.println("Distance to " + alg.nodes.get(11) + ": " + alg.nodes.get(11).minDistance);
//	 List<Vertex> path = Dijkstra.getShortestPathTo(alg.nodes.get(11));
//	 System.out.println("Shortest path: " + path);

	// Launch main window
	EventQueue.invokeLater(new Runnable()
	{
	    public void run()
	    {
		try
		{
		    MainWindow frame = new MainWindow();
		    frame.setVisible(true);
		    frame.setLocationRelativeTo(null);
		}
		catch (Exception e)
		{
		    e.printStackTrace();
		}
	    }
	});
    }

}
